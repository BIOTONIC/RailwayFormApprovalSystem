    ...
    if (person == '1') {
        ...
        if (nextperson == '10') {
            ...
            confService.getApplyCount().then((counts) => {
                if (typeof counts === 'undefined' || counts.length == 0) {
                    ...
                    app2Service.createApp2(app2).then((result) => {
                        ...
                        addOperationLog(req.session.userId, req.session.userName, req.session.person, loginIp, '二级审批表：新建表格');
                        return res.redirect('home');
                    }).catch((error) => {
            ...
            app2Service.updateApp2(app2).then((result) => {
                ...
                addOperationLog(req.session.userId, req.session.userName, req.session.person, loginIp, '二级审批表：更新表格');
                return res.redirect('home');
            }).catch((error) => {
                req.flash('error', '提交失败');
                return res.redirect('home');
            ...
            app2Service.updateResult(app2).then((result) => {
                ...
                addOperationLog(req.session.userId, req.session.userName, req.session.person, req.session.workshop, loginIp, '二级审批表：销点');
                return res.redirect('home');
            }).catch((error) => {
                req.flash('error', '提交失败');
                return res.redirect('home');
            ...
            confService.getApproveCount().then((counts) => {
                if (typeof counts === 'undefined' || counts.length == 0) {
                    ...
                    app2Service.updateWorkshopMgr(app2).then((result) => {
                        addOperationLog(req.session.userId, req.session.userName, req.session.person, req.session.workshop, loginIp, '二级审批表：车间审批');
